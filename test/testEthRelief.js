...
contract('EthRelief', accounts => {
  ...
  it('Fallback function should accept ETH', async () => {
    const contract = await ethRelief.deployed()
    let balance    = await ethRelief.web3.eth.getBalance(contract.address)
    assert.equal(balance.toNumber(), 0)
    await ethRelief.web3.eth.sendTransaction({to: contract.address, from: accounts[5], value: 10})
    balance = await ethRelief.web3.eth.getBalance(contract.address)
  ...
  it('This contract\'s balance moved entirely to upgrade contract', async () => {
    ...
    let balance    = await ethRelief.web3.eth.getBalance(contract.address)
    assert.equal(balance.toNumber(), 0)
    balance        = await ethRelief.web3.eth.getBalance(stub.address)
    assert.equal(balance.toNumber(), 10)
  })
